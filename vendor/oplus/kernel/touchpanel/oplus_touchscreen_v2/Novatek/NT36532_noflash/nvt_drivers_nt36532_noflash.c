...
static int nvt_reset(void *chip_data)
{
	...
	*/
	...
	if(ts->gesture_enable && ts->is_suspended == 1) {
		ret = nvt_cmd_store(chip_info, CMD_OPEN_BLACK_GESTURE);
		msleep(50);
		if (ts->health_monitor_support) {
			tp_healthinfo_report(&ts->monitor_data, HEALTH_SET_GESTURE_IN_SUSPEND, (void *)&ts->gesture_enable);
		}
		TPD_INFO("reset gesture value after nvt reset during suspending!\n");
	}
