...
static int op_rf_cable_probe(struct platform_device *pdev)
{
        ...
        if (rf_data->rf_cable_support_num) {
                proc_create_data("rf_cable", S_IRUGO, oplus_rf, (const struct proc_ops *)&cable_proc_fops_cable, rf_data);
        }

        if (rf_data->rf_pds_support_num) {
                proc_create_data("rf_pds", S_IRUGO, oplus_rf, (const struct proc_ops *)&cable_proc_fops_pds, rf_data);
        }
