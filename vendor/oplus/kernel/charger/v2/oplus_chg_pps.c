...
__maybe_unused
static int oplus_pps_cp_watchdog_enable(struct oplus_pps *chip, bool en)
{
	int rc;
	...
	chg_err("pps cp wd en:%d\n", en);

	rc = oplus_chg_ic_func(chip->cp_ic, OPLUS_IC_FUNC_CP_WATCHDOG_ENABLE, en);
...
static void oplus_pps_force_exit(struct oplus_pps *chip)
{
	...
	oplus_pps_cp_watchdog_enable(chip, false);
...
#define OPLUS_PPS_ABNORMAL_CURR (100)
	...
	if (curr_ma > curr_over_thd) {
		...
		rc = OPLUS_CURR_OK;
	} else if (curr_ma < OPLUS_PPS_ABNORMAL_CURR) {
		rc = OPLUS_CURR_ABNOR;
...
#define CP_MAX_VOLT_DELTA (500)
	...
	if (chip->need_check_current) {
		...
		case OPLUS_CURR_OVER:
			...
			if (chip->count.curr_over >= IBAT_OVER_ITARGET_CNT) {
		...
		case OPLUS_CURR_ABNOR:
			...
			if (chip->count.curr_abnormal >= IBAT_OVER_ITARGET_CNT) {
