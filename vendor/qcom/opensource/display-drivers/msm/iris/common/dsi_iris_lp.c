...
static u32 _regs_dump_i8[] = {
	...
	0xff97ffe4, //dsc_unit_decoder0
	0xff99ffe4, //dsc_unit_decoder1
	0xff43ffe4, //blending
};
...
void iris_dma_gen_ctrl(int channels, int source, bool chain)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		...
		iris_dma_gen_ctrl_i8(channels, source, chain);
}
/* control dma trigger
...
void iris_sleep_abyp_power_down(void)
{
	...
	pcfg->abyp_ctrl.preloaded = false;
	//Start: Don't refer HDK codes
	udelay(1000);
	//End: Don't refer HDK codes
}
...
void iris_global_var_init(void)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7) {
		...
		pcfg->id_piad_blend_info = ID_PIAD_BLEND_INFO_I7;
		...
		pcfg->id_piad_blend_info = ID_PIAD_BLEND_INFO_I7P;
		...
		pcfg->status_reg_addr = STATUS_REG_ADDR_I8;
		pcfg->id_sys_enter_abyp = ID_SYS_ENTER_ABYP_I8;
		pcfg->id_sys_exit_abyp = ID_SYS_EXIT_ABYP_I8;
		pcfg->ulps_mask_value = ULPS_MASK_VALUE_I8;
		pcfg->id_piad_blend_info = ID_PIAD_BLEND_INFO_I8;
		pcfg->te_swap_mask_value = TE_SWAP_MASK_VALUE_I8;
		pcfg->id_tx_te_flow_ctrl = ID_TX_TE_FLOW_CTRL_I8;
		pcfg->id_tx_bypass_ctrl = ID_TX_BYPASS_CTRL_I8;
		pcfg->id_sys_mpg = ID_SYS_MPG_I8;
		pcfg->id_sys_dpg = ID_SYS_DPG_I8;
		pcfg->id_sys_ulps = ID_SYS_ULPS_I8;
		pcfg->id_sys_abyp_ctrl = ID_SYS_ABYP_CTRL_I8;
		pcfg->id_sys_dma_ctrl = ID_SYS_DMA_CTRL_I8;
		pcfg->id_sys_dma_gen_ctrl = ID_SYS_DMA_GEN_CTRL_I8;
		pcfg->id_sys_te_swap = ID_SYS_TE_SWAP_I8;
		pcfg->id_sys_te_bypass = ID_SYS_TE_BYPASS_I8;
		pcfg->pq_pwr = PQ_PWR_I8;
		pcfg->frc_pwr = FRC_PWR_I8;
		pcfg->bsram_pwr = BSRAM_PWR_I8;
		pcfg->id_rx_dphy = ID_RX_DPHY_I8;
		pcfg->id_rx_init = ID_RX_INIT_I8;
		pcfg->iris_rd_packet_data = IRIS_RD_PACKET_DATA_I8;
		pcfg->iris_tx_intstat_raw = IRIS_TX_INTSTAT_RAW_I8;
		pcfg->iris_tx_intclr = IRIS_TX_INTCLR_I8;
		pcfg->iris_mipi_tx_header_addr = IRIS_MIPI_TX_HEADER_ADDR_I8;
		pcfg->iris_mipi_tx_payload_addr = IRIS_MIPI_TX_PAYLOAD_ADDR_I8;
		pcfg->iris_mipi_tx_header_addr_i3 = IRIS_MIPI_TX_HEADER_ADDR_I3_I8;
		pcfg->iris_mipi_tx_payload_addr_i3 = IRIS_MIPI_TX_PAYLOAD_ADDR_I3_I8;

		iris_global_var_init_i8();
	}
}
...
void iris_set_metadata(bool panel_lock)
{
	...
	IRIS_ATRACE_INT("iris_metadata", pcfg->metadata);
